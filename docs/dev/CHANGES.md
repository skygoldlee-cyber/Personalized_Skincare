...
> **목표**: `docs/dev/QUESTION_SCHEMA_DESIGN.md` + `content/문제은행/합답형 학습전략.md`를 실제 앱 파이프라인에 구현 — 진술(sid) 원자 단위로 O/X·합답형·약점 추적을 통합
...
2. **기출문제 링크 → ExamViewer 연동**: `reader-format.js`에서 기출문제 링크(`기출문제/과목N_...`)를 `data-exam-md` 속성으로 변환, 실제 파일 경로(`content/문제은행/과목N_문제.md`) 매핑
