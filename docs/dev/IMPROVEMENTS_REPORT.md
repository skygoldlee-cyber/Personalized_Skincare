...
>
> **2026-09-02 업데이트**: 이야기형 교재 4개 과목에 5가지 서사적 개선안 적용 — 프롤로그 강화, 읽는 방법 4단계 가이드, 여정도 추가, 실수→교훈 패턴(🔖 기억 태그), 에필로그(💭). 외부 리뷰 후 🎭 전환 블록, 🎬 통합 에필로그, 스마트쿼터 통일 추가. (섹션 11 추가, 11-6~11-8)
>
> **2026-09-02 추가 업데이트**: 교재 본문 기출문제 링크 클릭 시 앱 내 HTML 문제집 뷰어(ExamViewer)로 이동. 마크다운 `[text](url)` 링크 파싱 추가, 참조자료 PDF 링크도 앱 내 뷰어로 연동. (섹션 12 추가)
...
### 11-2. ✅ 읽는 방법 4단계 가이드
- 상황 이해 → 🎯 시험 포인트·🧠 암기 포인트 확인 → 🔖 기억 태그로 함정 회피 → ⚖️ 법령 원문 확인 → 💭 에필로그를 읽으면 다음 장이 예고되어 흐름 연결
- 학습 아이콘 표(🎯⭐🧠) 추가
- 외부 리뷰 후: "에필로그와 다음 장 예고" → "에필로그를 읽으면 다음 장이 예고되어"로 문구 다듬기 (3개 과목)
...
### 11-6. ✅ 🎭 "이야기에서 자료로" 전환 블록 (외부 리뷰 후 추가)
- 4개 과목 별표·별지 서식 종합 인덱스 앞에 서사→실무 자료 전환 안내 블록 추가
- "여기서부터는 인물의 장면이 아니라 근거 자료"임을 명시하여 독자 안내
- 각 과목 주인공(민수·지연·현우·수진)의 시점으로 전환 메시지 작성

### 11-7. ✅ 🎬 통합 에필로그 — CÔTELEAF의 첫 제품 (외부 리뷰 후 추가)
- 4과목 말미에 4개 과목 네 주인공이 하나의 조제대에서 만나는 통합 에필로그 추가
- 법(1과목) → 원료·품질(2과목) → 유통·안전(3과목) → 맞춤형 조제(4과목)로 이어진 여정을 하나의 제품이 고객에게 닿기까지의 흐름으로 마무리
- `🏁 이야기 끝 — 이제 당신의 차례입니다.`로 학습자에게 마지막 메시지 전달

### 11-8. ✅ 스마트쿼터 → ASCII 따옴표 통일 (외부 리뷰 후 추가)
- 법령 원문 및 에필로그 대사의 스마트쿼터(“”)를 ASCII 따옴표(")로 통일 (3·4과목)
- 인코딩 일관성 확보
...
## 섹션 12: 교재 본문 기출문제 링크 → 문제집 HTML 뷰어 연동 (2026-09-02)

### 12-1. ✅ 마크다운 링크 파싱 추가
- `markdown-parser.js`에 `[text](url)` → `<a href="url">text</a>` 변환 로직 추가
- 기존에는 마크다운 링크 문법이 텍스트로만 렌더되어 클릭 불가였음
- 인라인 서식(강조/코드) 처리 후, 블록 파싱 전 단계에서 수행

### 12-2. ✅ 기출문제 링크 → ExamViewer 연동
- `reader-format.js`에서 기출문제 링크(`기출문제/과목N_...`)를 `data-exam-md` 속성으로 변환
- 상대 경로를 실제 파일 경로(`content/문제은행/과목N_문제은행_교재인용.md`)로 매핑
- 버튼 스타일 적용 (파란색 배경, 펜 아이콘)으로 시각적 클릭 유도

### 12-3. ✅ 참조자료 PDF 링크 처리
- 표 내 `[file.pdf](../참조자료/...)` 형태의 링크를 `data-ref-html`로 변환
- `resolveRefPath()`를 통해 HTML 뷰어 경로로 변환하여 앱 내에서 열리도록 처리
- 기존 백틱 기반 PDF 링크 처리와 중복 없이 동작

### 12-4. ✅ 클릭 이벤트 바인딩
- `textbook-reader.js`의 `bindReferenceLinks()`에 `data-exam-md` 클릭 핸들러 추가
- `ExamViewer.openExam(mdPath)` 호출로 앱 내 전체화면 오버레이에 문제집 HTML 렌더링
- 교재 리더에서 벗어나지 않고 오버레이로 문제집 열람
