...
4. **다중 빈칸 문항** — `(A)`만 1문항으로 변환 (정답 = `answer` 쉼표 분할의 첫 번째).
   총량 1,000문(100/250/250/400) 정책으로 `(B)`는 생성하지 않는다 — `(B)` 분리 생성이 필요하면
   `buildBlankCombo(q, 'B', …)` 호출을 추가로 붙이면 된다 (라벨 파라미터는 seed/sid 안정성용으로 유지).
