...
**변환 결과**: 과목1 `100` / 과목2 `234` / 과목3 `255` / 과목4 `305` = **894문**
(원본 변환 746 + 개념 재조합 9 + 참조자료 원문 139), 스키마 검증 오류 0.
...
- 공백 소실 문서(공백 비율 <7%, 시행규칙 별표·KFCC 별표 등 17건) 제외
- 셀 절단 손상 표(번호 없는 원료 표) 제외 — 이름 조각이 진술이 되므로
- 멤버 필터: 4~140자, 한글 ≥40%, 균형 괄호, 포괄조항(`그 밖의` 등)·특수기호·조각 종료 제외
...
**산출(2026-09-20 기준)**: 참조자료 문항 s1 35 / s2 76 / s4 28 = 139문,
