...
> **버전**: 1.9 (2026-09-26 기준 — §7 향후 고도화 로드맵 신설, DA-09 저장소 추상화, Learning Pro 모듈·부록 반영)
...
| D-11 | 오늘의 합격 전략 — 우선순위 추천 엔진 (SM-2 대기 → 과락 과목 → 정답률 최저 → 헷갈린 카드 → 미학습), 이유 표시 + 즉시 실행 버튼 (`recommendations.js`) | ✅ |
| D-12 | 시험일 D-day 카드 + 역산 일일 권장 카드 수 (목표 설정 모달에서 시험일 입력, 캘린더 D-day 칩) | ✅ |
| D-13 | 예상 점수 추정 — 최근 모의고사 5회 평균 ±1σ + 선형회귀 추세(상승/보합/하락), 합격 진단 카드 하단. "합격 확률"이 아닌 점수 추정치로 표기 한정 | ✅ |
| D-14 | 실제 시험 결과 자가 보고 (`actual_exam_result`) — D-day 경과 시 진단 카드에 입력 폼, 기록 후 예상 대비 오차 표시. 예측 보정 데이터 수집 경로 | ✅ |
| D-15 | 대시보드 추천 카드에서 "맞춤 리포트 보기" 버튼으로 `analysis-view` 연결 (퍼널) — 상세 분석(과목 카드·히트맵·모의고사 차트)은 맞춤 학습 리포트 뷰로 분리됨 | ✅ |

### 3.1.5 맞춤 학습 리포트 (Personal Analysis, `analysis-view`)
...
| F-10 | 난이도 필터 (전체/쉬움/보통/어려움, `difficultyFilter` 상태) | ✅ |
...
| Q-08 | 오답 → 약점 자동 수집 (`weak_quiz_*` ID) — 복습 노트·약점 퀴즈 연동 | ✅ |
| Q-09 | 오답 원인 자가 태깅 (암기부족/개념오해/계산실수, `quiz_wrong_causes` 영속) + 재학습 링크 (복습 노트·교재·유사 문제) | ✅ |
| Q-10 | 진단 평가 — 전 과목 균등 샘플링(~20문) → 과목별 약점 프로파일 + 집중 퀴즈 링크 | ✅ |
| Q-11 | 오답 패턴 분석 — 최근 7일 원인 분포·권장 학습법 (복습 노트 상단) | ✅ |
...
| TR-14 | 이야기형 모드 토글 (`storyMode` 상태, 표준형 ↔ 이야기형 MD 전환, `localStorage` 영속화) | ✅ |
| TR-15 | 계층형 TOC (챕터/섹션 접기·펼치기, 하위 헤딩 표시) | ✅ |
| TR-16 | 모바일 TOC 드로어 (왼쪽 끝 엣지 스와이프 오픈, 엣지 힌트 탭, 백드롭/왼쪽 스와이프/항목 클릭 시 닫기) | ✅ |
...
| ID | 요구사양 | 구현 상태 |
|----|---------|-----------|
| FO-01 | 배합 계산기 — 총 제조량(g/ml) × 배합률(%) → 원료별 투입량 자동 계산, 합계 100% 판정 배지 | ✅ |
| FO-02 | 고시 한도 실시간 규정 검증 — 행별 한도이내/초과/금지/확인필요 4상태 + 상단 요약 건수 (`formula-check.js`) | ✅ |
| FO-03 | 제조 단계(Phase) 태그 — 수상부/유상부/실리콘부/기능성/후첨가/기타, 단계별 소계·정렬 | ✅ |
| FO-04 | 고객 정보 (접이식) — 고객명·나이·성별·피부 유형·제형·고민 칩 + 안전 필드(알레르기 이력·임신수유·사용 중 제품/약물) | ✅ |
| FO-05 | 규칙 기반 추천 — 제형별 베이스 역할 + 고민/피부 유형별 원료 제안 (이름만, 농도 미제안), 안전 필터(banned·알레르기 제외, 임신 ⚠) | ✅ |
| FO-06 | 맞춤 추천 규칙 — 사용자 후보 추가/삭제/초기화, JSON 내보내기·가져오기 (기본 규칙에 병합) | ✅ |
| FO-07 | 제조 정보 — 목표/실측 pH, 제조 절차 단계(최대 20), 메모 | ✅ |
| FO-08 | My 포뮬러 — 저장(최대 5개)·열기·복제·삭제, 규정 스냅샷 보존, 백업 포함 | ✅ |
...
| FO-15 | **허브 6개 메뉴 + 서브내비 칩** — 업무 흐름 순(고객 관리·배합 계산기·My 포뮬러·조제 기록·원료 장부·법규 준수), 모든 서브패널 상단에 동일 칩 바(`formulaSubNav`)로 상호 이동 | ✅ |
...
| ID | 요구사양 | 근거·구현 포인트 |
|----|---------|------------------|
| UX-SCR-01 | **포인터 타입별 이원화**: 마우스 환경은 굵고 대비 높은 스크롤바(10px, 진한 thumb, `--bg-subtle` 트랙), 터치 환경은 완전 숨김 | `@media (pointer: coarse), (max-width: 900px) { * { scrollbar-width: none } ::-webkit-scrollbar { width:0; height:0 } }` — `pointer: coarse`만 믿지 말고 폭 기준을 병기할 것(일부 기기에서 pointer 감지 실패 사례 있음). 모바일 스크롤바는 드래그용이 아니므로 위치 표시도 불필요 |
| UX-SCR-02 | **`scrollbar-width`는 스크롤 컨테이너 각각에 적용됨**(Firefox): `html`에만 주면 내부 컨테이너에 미적용 | `*` 또는 개별 컨테이너에 지정 |
| UX-SCR-03 | **커스텀 색상은 CSS 변수로**: `::-webkit-scrollbar-thumb { background: var(--scrollbar-thumb) }` — 테마별 변수 정의 | 하드코딩 색상은 다크/라이트 한쪽에서 묻힘 (실제로 미정의 변수 폴백으로 라이트 배경에 흰 카드가 되는 사고 있었음 — `.comp-item` 사례) |
...
| ID | 요구사양 | 근거·구현 포인트 |
|----|---------|------------------|
| UX-FB-01 | **토스트는 모바일에서 하단 배치**: 중앙 팝업은 읽던 콘텐츠를 가림 | `@media (max-width: 768px) { #app-toast { top:auto; bottom: calc(80px + safe-area) } }` — 하단 탭 바 위 |
...
| ID | 요구사양 | 근거·구현 포인트 |
|----|---------|------------------|
| UX-FORM-01 | **iOS 입력 줌 방지**: 입력 필드 `font-size ≥ 16px` | 16px 미만이면 iOS Safari가 포커스 시 자동 확대 |
| UX-FORM-02 | **터치 피드백**: `:active` 상태에 `transform: scale(0.92~0.98)` | 모바일에서 물리적 눌림감 제공 |

#### 4.8.7 알려진 함정 (실제 사고 기록)

| 함정 | 원인 | 교훈 |
|------|------|------|
| 다크모드에서 카드가 흰 배경+회색 글자 | CSS 변수 미정의 → 폴백 `#fff` 상시 적용 | `var(--x, fallback)`의 폴백이 "항상" 쓰이지 않는지 grep으로 변수 정의 확인 |
| 모바일에서 버튼이 아예 안 보임 | 조상(`.sidebar-footer`)이 모바일 미디어쿼리에서 `display:none` | 요소를 추가할 때 모든 미디어쿼리에서의 가시성 확인 — 숨겨지는 컨테이너 밖에 배치 |
...
> 이 섹션은 **미구현 항목**의 정규 요구사양이다. 상태 열의 ✅가 아닌 모든 항목은 현재 구현되지 않았다. 각 항목은 "전환 트리거"(도입을 정당화하는 조건)를 함께 명시한다 — 트리거 충족 전 도입은 금지 (불필요한 복잡도/페이로드 증가 방지).

### 7.1 저장소 고도화 (DA-09의 확장)

| ID | 요구사항 | 전환 트리거 | 상태 |
|----|---------|------------|------|
| ROAD-S1 | **IndexedDB 백엔드**: `storage.js`의 `setStorageBackend()`로 교체 가능한 IndexedDB 구현체 (`sync:false` + Async 메서드) | 배치/판매 기록이 수천 건 규모로 성장, quota 에러 실측 | 미구현 |
| ROAD-S2 | **호출부 Async 이행**: 신규 코드는 `loadItemsAsync`/`saveItemsAsync`만 사용, 기존 동기 호출부를 점진 이행 후 백엔드 교체 | ROAD-S1 착수 시 | 진행중(신규 코드 권장 경로 확보 완료) |
| ROAD-S3 | **SQLite WASM 검토**: 공식 sqlite-wasm + OPFS + Worker. wasm ~1.5MB 페이로드·`wasm-unsafe-eval` CSP 추가·Worker 아키텍처 필요 | append-only 감사 장부 + 건별 기록 간 join·집계 리포트가 실제 요구사항이 될 때 (Phase 3+) — IndexedDB가 중간 단계로 우선 | 보류 |
| ROAD-S4 | **다중 엔티티 원자성 확장**: `setMany`/`setJSONMany` 롤백을 배치 기록+재고 차감+고객 이력 등 다중 키 갱신 경로로 확대. 진짜 트랜잭션은 IndexedDB 전환 시 | Formula OS 다중 엔티티 갱신 지점 추가 시 | 부분 적용 (custDelete) |

### 7.2 유료화 인프라 (Premium 출시 선행)

> 상세 설계: `SUBSCRIPTION_ROADMAP.md` §3~§6, `LEARNING_PREMIUM_PLAN.md` D 섹션. **지연 원칙**: 제품 가치 검증(사용률·유료 전환 의향) 전 결제 인프라 구축 금지.
...
| ROAD-L5 | **기능 검증 측정**: 개발 완료 기능(오답 루프·합격 전략·진단 평가)의 사용률·만족도 로컬 측정 → 유료가치 판정 데이터 확보 | LEARNING_PREMIUM_PLAN "기능 검증 상태" 표 | 측정 필요 |
...
1. **가치 검증 우선**: ROAD-L5(사용률 측정) → 유료 전환 의향 → ROAD-P1~P4(결제 인프라). 결제 인프라를 가치 검증 전에 구축하지 않는다
2. **트리거 기반 전환**: 저장소 백엔드(ROAD-S1/S3)는 규모·요구사항 트리거 충족 시에만. localStorage는 현재 규모에서 최적
3. **구조 선행**: 신규 코드는 Async 스토어 API·약점 ID 문법·`scopedKey` 등 확립된 추상화 경로를 통해 작성 — 미래 전환 비용을 선제적으로 절감
