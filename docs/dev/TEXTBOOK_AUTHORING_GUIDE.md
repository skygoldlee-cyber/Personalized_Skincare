...
> **이야기형 프론트 매터**: 이야기형 파일은 표준형 프론트 매터 앞에 서사용 섹션을 추가한다.
>
> | 순서 | 헤더 | 아이콘 | 용도 |
> |---|---|---|---|
> | 0a | `##` | (없음) | 프롤로그 (이야기 도입) |
> | 0b | `##` | 🧭 | 등장인물 소개 |
> | 0c | `##` | 🧭 | 여정도 (이야기 흐름 한눈에 보기) |
>
> 이후 표준형과 동일한 순서로 이어진다:
> 🧭 학습 아이콘 → 🚀 시험 직전 → 🎯 최우선 암기 축 → 🔢 숫자 암기 → 📋 목차 → 🎯 과목 시각화
...
> **챕터 헤더 규칙**:
> - `📚` 이모지 **필수** (표준형·이야기형 공통, `## Chapter 01.` 형식 금지)
> - 이야기형은 서사 부제(`— ...`) 추가 (`## 📚 Chapter 01. 제목 — 부제`)
...
> **번호체계 상세**: [`docs/dev/NUMBERING_SYSTEM.md`](./NUMBERING_SYSTEM.md) 참조
...
> **📌 용어 정리 섹션 규칙**:
> - 각 챕터마다 **정확히 1개**의 `## 📌 용어 정리` 섹션을 배치한다 (중복 금지).
> - 표준형과 이야기형은 **동일한 용어 세트**를 유지한다 (용어명, 항목 수 일치).
> - 영어 약어는 **full name 병기** (예: `CGMP(Cosmetic Good Manufacturing Practice)`).
> - `###` 하위 섹션으로 빈출 표시: `(빈출)`, `(빈출 최상위)` 등.
...
인식 단위: `%`, `세 이하`, `세 이상`, `개월`, `일`, `년`, `배`, `종`, `가지`, `개`, `시간`, `g`, `ml`, `kg`, `℃`, `도`, `분`, `초`, `주`, `ppm`, `㎛`, `회/hr`, `개/hr`, `개/㎥`.
숫자와 단위 사이 공백 허용(`120 ℃`). 단, 단위 바로 뒤에 한글·영숫자가 붙은 경우는 매칭하지 않는다(`3분기`의 "3분", "2026년도"의 "2026년" 같은 어중간한 분절 방지).

#### 생성 제외 규칙

- **질문 최소 길이**: 빈칸형 15자 이상 / term형(정의→용어) 11자 이상 — 카드의 `정의 >10자` 규칙과 정합
- **모호한 term형**: 같은 정의를 공유하는 용어가 둘 이상이면(예: `징크옥사이드`/`티타늄디옥사이드` 둘 다 "25%(자외선 차단 성분으로)") 정답 타이핑 채점이 불가하므로 해당 term형 퀴즈는 생성되지 않는다. 내용 자체는 카드로 계속 노출된다.
- **라벨 라인**: 마커가 붙어도 퀴즈/경고 대상이 아닌 라인 — `###`/`####` 헤더, `**항목명**`만 있는 볼드 라벨, `④ 항목명` 같은 번호·불릿 라벨. 퀴즈를 원하면 본문/표/리스트 라인에 마커를 붙이고 빈칸 대상(`**볼드**` 또는 숫자+단위)을 포함시켜야 한다.
...
#### 두음 암기법 (Acrostic Mnemonics)
```markdown
> **🧠 두음법**: **법·시·규·고** (법률→시행령→시행규칙→고시)
```

> **두음법 작성 규칙**:
> - 각 항목의 첫 글자를 조합해 한 단어/문장으로 만든다
> - `한 줄 요약` 다음 줄에 `🧠 두음법` 형식으로 배치
> - 표준형/이야기형 동일하게 적용
> - 핵심 암기 항목이 3개 이상인 섹션에 권장

#### 출처 표기
```markdown
> 📌 **출처**: 화장품법 제1조| **참조 PDF**: [화장품법(법률)(제20901호)(20260402).pdf](../참조자료/법령고시/화장품법%28법률%29%28제20901호%29%2820260402%29.pdf)| **시행일**: 2026. 4. 2.| **최종 확인**: 2026. 8. 29.
```
...
#### ref_md 원문 링크
```markdown
> **원문 보기**: [화장품법(법률)(제20901호)(20260402).pdf](../참조자료/ref_md/과목1/화장품법(법률)(제20901호)(20260402)/화장품법(법률)(제20901호)(20260402).md)
```
...
#### 3.7.2 마인드맵 노드 상세 매핑 표

각 챕터의 첫 번째 개요 마인드맵 다음에는 `> **🗺️ 마인드맵 노드 상세 매핑**` 인용 블록과 함께 매핑 표를 작성한다.

**형식 규칙**:
- **3단 표** (대분류 | 중분류 | 핵심 포인트) — 모든 매핑 표는 3단으로 통일
- **배치**: 챕터 헤딩(`## 📚 Chapter NN.`) 직후, 본문 `## 1.` 섹션 앞
- **대분류**: 마인드맵 root 직하위 노드와 일치
- **중분류**: 마인드맵 2단계 노드. 소분류가 필요한 경우 `·` 구분자로 중분류에 병합 (예: `제1절 시설기준 · 제7조 건물`)
- **핵심 포인트**: 해당 노드의 핵심 요약
- **"해당 없음" 금지**: 빈 셀은 공백(`|  |`)으로 처리. "해당 없음" 텍스트 사용 금지
- **대분류 병합**: 동일 대분류의 연속 행은 첫 행만 대분류를 표시, 나머지는 공백
- **중분류 필수**: 모든 행은 중분류에 의미 있는 값을 가져야 함. "개요" 사용 금지. 마인드맵이 flat 구조(대분류 직하위에 중분류가 없는 경우)일 때는 공통 대분류를 설정하고 각 항목을 중분류로 이동
...
| 대분류 | 중분류 | 핵심 포인트 |
| --- | --- | --- |
| 법령 체계 | 법률 | 화장품법: 1999년 약사법에서 분리 |
|  | 시행령 | 대통령령: 법에서 위임된 사항 규정 |
|  | 시행규칙 | 총리령: 세부 시행절차 규정 |
```

#### 3.7.3 Flowchart (플로우차트)
...
정답의 교재 근거는 인용 링크로 연결한다. 인용 링크는 라인 번호 기반 하이퍼링크 형식을 사용한다:

```markdown
> **정답: ③** (교재: [L250](<../교재/law/1과목_화장품법의이해_표준형.md#L250>))
>
> **📖 교재 근거 ([L250](<../교재/law/1과목_화장품법의이해_표준형.md#L250>)):**
> {교재 원문 또는 표}
```
...
```
[label: L####](<상대경로/파일명.md#L####>)
```

- `label`: 링크 텍스트 (예: `L250`, `교재 L250`)
- `L####`: 라인 번호 (1-indexed)
- `상대경로`: 문제은행 파일 기준 상대 경로 (예: `../교재/law/`)
...
- **검증 순서**: 인용 라인 ±3에 인용문 조각(4자 이상) 존재 확인 → 실패 시 전체 파일 재탐색(정확 포함 → 쉥글 2개 이상 → 키프레이즈)
- **범위초과**: 구 라인이 파일 길이를 넘어도 인용문 지문으로 재탐색 (skip하지 않음)
- **메타 라인 제외**: 📖 근거 헤더, 해설, TIP, 허용 정답 라인은 인용문 지문에서 제외
- **낮은 신뢰도**: 키프레이즈만 일치하는 경우 자동 갱신하지 않고 미발견으로 보고
- **미발견 항목**: 수동 확인 필요 (exit code 1)
- `npm.cmd run build:data` 실행 시 `sync:citations` 스크립트가 자동으로 포함됨
...
이야기형 파일은 표준형과 동일한 `## 📚 Chapter NN.` 챕터 헤더와 `##` 섹션 구조를 유지하되, 각 섹션 시작에 서사(이야기)를 추가한다:
...
> 📌 **출처**: 화장품법 제1조| **참조 PDF**: [화장품법(법률)(제20901호)(20260402).pdf](../참조자료/법령고시/화장품법%28법률%29%28제20901호%29%2820260402%29.pdf)| **시행일**: 2026. 4. 2.
...
- **`## 📚 Chapter NN.` 헤더 사용**: 이야기형도 표준형과 동일하게 `## 📚 Chapter NN. 제목 — 서사 부제` 형식 사용
...
빌드 시 `[마커 감시]` 경고가 나오면 `🔖기출` 마커가 있는데 퀴즈가 생성되지 않은 항목이 있다는 뜻이므로, 해당 항목의 볼드/숫자 패턴을 확인하라. 경고는 `파일:L<라인번호>` 형태로 위치를 표시한다. (헤더·라벨 라인의 마커는 경고 대상이 아니다 — §3.5 "생성 제외 규칙" 참조)
...
- [ ] `🔖기출` 마커가 헤더·라벨 라인이 아닌 실제 내용 라인에 붙어 있는가? (라벨 라인은 퀴즈 미생성 + 경고 안 뜸)
- [ ] 문제은행 정답 섹션이 `## 🔑 정답 및 교재 근거` 아래에 있는가?
- [ ] 이야기형 파일을 작성하는 경우, 파일명이 `{기본파일명}_이야기형.md`인가?
- [ ] 이야기형 파일에 `## 📚 Chapter NN.` 헤더가 있는가? (서사 부제 포함)
- [ ] mindmap 블록의 들여쓰기가 계층 구조를 반영하는가? (root 1 space, 각 레벨 +1 space)
- [ ] 각 챕터마다 `## 📌 용어 정리` 섹션이 **정확히 1개**인가?
- [ ] 표준형과 이야기형의 `📌 용어 정리` 용어 세트가 일치하는가?
- [ ] 영어 약어에 full name이 병기되어 있는가? (예: `CGMP(Cosmetic Good Manufacturing Practice)`)
