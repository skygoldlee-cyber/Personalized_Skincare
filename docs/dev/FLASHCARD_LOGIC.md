...
플래시카드 시스템은 교재 Markdown(`content/exams/<id>/교재/**/*.md`)에서 표(table)와 리스트 항목(list item)을 파싱하여 **용어-설명 카드**를 자동 생성한다. 각 카드는 중요도 점수에 따라 필터링되며, 8가지 타입으로 분류되어 프론트엔드에서 배지로 표시된다.
