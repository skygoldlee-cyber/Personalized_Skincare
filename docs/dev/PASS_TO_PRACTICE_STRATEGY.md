...
"건조하고 민감한 피부"
  ↓
[AI: 제안] 사용 가능한 원료 검색 (사용 가능/제한 원료 필터)
  ↓
[AI: 제안] 원료 특성 분석 (보습력, 자극도, 궁합)
  ↓
[AI: 제안] 원료 조합 후보 생성 (3~5개 후보, 배합량 미포함)
  ↓
[사용자: 입력] 각 원료의 배합량을 사용자가 직접 입력
  ↓
[시스템: 검증] 배합량이 법정 한도 내인지 자동 검증 (공식 고시 데이터)
  ↓
[시스템: 검증] 궁합 충돌, 알레르기 향료, pH 충돌 자동 경고
  ↓
[사용자: 결정] 최종 배합 확정 및 조제 기록
```
...
관련 포뮬러 연결
  ↓
[검증 게이트] 공식 데이터/학술 근거와 교차 검증
  ↓
검증 통과 시: 실무 데이터 자산 축적 (verified = true)
검증 미통과: 커뮤니티 경험으로만 분류 (verified = false)
  ↓
실무 데이터 자산 축적
```
...
실무 사용자
    ↓
포뮬러 데이터 (원료 조합, 배합량, 고객 반응)
    ↓
원료/제형 데이터 축적
    ↓
[검증 게이트] 공식 데이터/학술 근거와 교차 검증
    ↓
검증된 데이터만 AI 학습 데이터로 승격
    ↓
AI 배합 참고 정확도 향상 (검증된 데이터만)
    ↓
실무 가치 증가
    ↓
사용자 증가
...
2순위 (Supabase 연동 후, 법률 검토 완료 후)
├── AI 배합 참고 (생성·검증 분리 구조, 법률 검토 필수)
├── 포뮬러 버전 관리
├── 고객별 조제 기록
└── 법령·고시 업데이트 알림
