...
> ⚠️ **현재 배포본에 이미 존재하는 위험**: Phase 2는 포뮬러·배치·원료를 이미 `sync_snapshots` LWW로 동기화 중이다. 건별로 쌓이는 기록(배치·원료 입고)은 다기기 동시 기록 시 충돌 모달에서 한쪽이 유실된다. **즉시 조치 후보 (게이트 무관)**: 충돌 해결 시 미선택 쪽 스냅샷을 삭제하지 말고 로컬 백업 키 또는보내기 파일로 보존 — `sync.js` 충돌 분기에서 구현 가능, 코드 소규모.
