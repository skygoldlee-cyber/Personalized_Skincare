...
> ⚠️ **LWW 유실 위험 — 부분 완화됨 (2026-09-25)**: Phase 2는 포뮬러·배치·원료를 이미 `sync_snapshots` LWW로 동기화 중이라, 건별로 쌓이는 기록은 다기기 동시 기록 시 충돌 모달에서 한쪽이 유실될 수 있었다. **즉시 조치 적용**: `sync.js` 충돌 분기에서 미선택 쪽 스냅샷을 `sync_conflict_backup` 키에 보존(선택 시점 기준) — 사용자가 복구할 수 있도록 별도 복원 UI는 후속 과제. append-only 행 단위 테이블(아래 결정 ①)이 구조적 해결.
