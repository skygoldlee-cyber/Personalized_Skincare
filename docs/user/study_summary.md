...
```
[표피의 5대 층 구조 (바깥쪽부터)]
각질층 (Stratum corneum)  <- 피부 장벽 역할, TEWL 방지
    ↓
투명층 (Stratum lucidum)   <- 손바닥, 발바닥에만 존재 (엘라이딘 함유)
    ↓
과립층 (Stratum granulosum) <- 케라토히알린 과립 존재 (수분 증발 방지막)
    ↓
가시층 (Stratum spinosum)  <- 표피 중 가장 두꺼움, 면역 담당(랑게르한스 세포)
    ↓
기저층 (Stratum basale)    <- 세포 분열(각화 주기 28일), 멜라노사이트 존재
```
