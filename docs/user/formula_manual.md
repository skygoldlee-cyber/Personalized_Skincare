...
> **최종 업데이트**: 2026-09-24 · **버전**: v369 · 시험 학습 기능은 [학습 매뉴얼](doc:user_manual) 참조
...
*   **허브 카드 6개**: 고객 관리 · 배합 계산기 · My 포뮬러 · 조제 기록 · 원료 장부 · 법규 준수 — 아래 업무 처리 흐름 순서와 동일하게 배치됩니다.
    *   성분 검색 사전은 사이드바(모바일은 학습도구 탭)의 별도 메뉴로 열립니다 — 원료 상세에서 '포뮬러에 추가'로 계산기에 바로 투입 가능합니다.
...
*   **서브내비 칩**: 각 화면 상단의 칩으로 허브 복귀 없이 다른 섹션으로 바로 이동합니다 (고객 관리 / 배합 계산기 / My 포뮬러 / 조제 기록 / 원료 장부 / 법규 준수)

### 🔄 업무 처리 흐름 (Workflow)

```mermaid
graph TD
    A["1. 고객 관리<br/>고객 카드·상담 이력"] -->|고객 카드 불러오기| B["2. 배합 계산기<br/>처방 작성·규정 검증"]
    ...
    E -->|판매내역서·기록 보관| F
    E -->|기록지·라벨·안내문 인쇄| G["5. 출력물<br/>기록지·라벨·안내문"]
...
## 2. ⚗️ 배합 계산기 (Blending Calculator)
...
*   모든 Formula OS 데이터는 **이 기기의 로컬 저장소**가 원본입니다. 로그인하면 포뮬러·배치·원료·체크리스트 등은 클라우드와 동기화되지만, **고객 카드·상담 이력(타인 개인정보)은 동기화에서 제외**되어 이 기기에만 남습니다.
*   **⚙️ 설정 → 데이터 백업**에 포뮬러·고객·배치·원료·체크리스트가 자동 포함됩니다 — 기기 변경 전 반드시 백업하세요 (고객 데이터는 동기화되지 않으므로 백업이 유일한 이동 수단).
