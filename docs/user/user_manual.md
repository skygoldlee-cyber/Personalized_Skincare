...
시험 학습을 실무 배합으로 잇는 작업 공간입니다. 원료를 조회하고, 배합률을 계산하고, 나만의 포뮬러를 저장·검증할 수 있습니다.

### 배합 계산기 (Blending Calculator)

*   **실제 투입량 자동 계산**: 총 제조량(g/ml)과 원료별 배합률(%)을 입력하면 원료별 실제 투입량과 합계가 즉시 계산됩니다.
*   **원료 자동완성**: 원료명 입력 시 1,357종 데이터베이스에서 자동완성을 지원합니다.
...
*   맞춤 조제 대상의 **고객명·나이·성별·피부 유형·선택 제형**을 입력하고 **피부 고민·증상**을 칩으로 복수 선택할 수 있습니다.
*   고객 정보는 포뮬러와 함께 저장되며, My 포뮬러 목록 카드에 요약 표시됩니다. 모두 선택 항목이므로 비워 두어도 저장됩니다.

### My 포뮬러 (저장)

*   **포뮬러로 저장**: 이름과 메모를 붙여 현재 배합 구성을 저장합니다 (Free 최대 5개).
*   **목록 관리**: 저장된 포뮬러를 열어 수정하거나, 복제·삭제할 수 있습니다. 목록 카드에 규정 검증 요약이 표시됩니다.
