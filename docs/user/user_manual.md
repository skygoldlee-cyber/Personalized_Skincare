...
*   **⚙️ 설정 메뉴**: 화면 우측 상단의 톱니바퀴 버튼을 누르면 관리 기능이 한곳에 모여 있습니다 — 데이터 백업, 백업 가져오기, 시험 전환, 학습 매뉴얼·실무 매뉴얼, **학습/실무 모드 전환**, **앱 종료**, 진도 초기화, **변경 이력**, **의견 보내기**. 메뉴는 항목을 선택하거나 바깥을 누르면 자동으로 닫힙니다.
...
*   **화면 구성**: 첫 화면에는 핵심 통계 카드, 일일 데일리 챌린지, 학습 안내서, **오늘의 맞춤 학습 추천**이 보입니다. 뽀모도로 타이머는 아래의 **"집중 도구"** 바를 눌러 펼쳐서 사용합니다. 상세 분석(과목별 상태·정답률 히트맵·모의고사 예측·오답 패턴)은 사이드바의 **"🎯 맞춤 학습 리포트"** 메뉴에 별도 화면으로 분리되어 있습니다.
...
학습 데이터를 종합해 **지금 가장 먼저 할 일**을 우선순위로 추천합니다. 대시보드에 항상 표시되며, 각 추천에는 "왜 이걸 먼저 해야 하는지" 이유가 함께 표시되고, 버튼을 누르면 바로 해당 학습이 시작됩니다. 우측의 **"맞춤 리포트 보기 →"** 버튼을 누르면 [맞춤 학습 리포트](#15-🎯-맞춤-학습-리포트-personal-analysis) 화면으로 이동합니다.
...
*   📚 과목별 학습 우선순위와 시간 배분 추천은 **[학습 안내서](/content/exams/cosmetic/docs/학습안내서.md)**의 "5. 추천 학습 방법" 섹션을 참조하세요.
...
## 1.5. 🎯 맞춤 학습 리포트 (Personal Analysis) <span class="pro-badge">PRO</span>

사이드바 **학습 → 맞춤 학습 리포트** 메뉴로 진입합니다. 흩어져 있던 개인화 분석을 한 화면에 모은 전용 화면입니다. **Pro 버전에서 제공되는 기능**으로, 첫 진입 시 1회 안내가 표시됩니다(현재는 무료 체험으로 누구나 이용 가능).

### 🧭 학습 진단 요약

화면 상단에 내 학습 상태를 진단하는 3개 카드가 표시됩니다.

*   **오답 패턴 분석**: 최근 7일간 오답 복습에서 태그한 "틀린 이유"(암기 부족·개념 오해·계산 실수) 분포와 권장 학습법을 보여줍니다. 태그된 오답이 없으면 오답 복습으로 이동하는 버튼이 표시됩니다.
*   **취약 진술 추적**: O/X·복수정답형 드릴에서 반복 오판한 진술을 보여줍니다. 오늘 복습 대기 수와 오판 횟수 상위 진술이 표시되며, "취약 리뷰 열기"로 스마트 훈련소의 취약 리뷰 패널로 바로 이동합니다.
*   **학습 리듬**: 오늘 목표 달성률, 이번 주 학습일, 시험일 D-day를 요약합니다. "캘린더 보기"로 학습 캘린더로 이동합니다.
...
## 5. 📝 실전 예상문제집 & OMR 시뮬레이터 (Mock Exam) 💎 PRO

> 💎 **PRO 제공 예정 기능**: 모의고사·복수정답형 문제집은 Pro 버전 제공 예정입니다. 현재는 무료 체험 기간으로 누구나 이용할 수 있습니다 (진입 시 안내 1회 표시).
...
graph TD
    ...
    Result --> Pass{"평균 60점 이상<br/>+ 전 과목 40점 이상?"}
    Pass -->|예| Done["✅ 합격 예상"]
    Pass -->|아니오| Fail["⚠️ 평균 미달 또는 과락 발생<br/>→ 맞춤 보완 학습 버튼"]
```
...
> 📚 **학습 전략 상세 안내**: 출제비중 기반 우선순위 등급(S/A/B/C), 과목별 학습 순서, 시간 배분 추천 등은 **[학습 안내서](/content/exams/cosmetic/docs/학습안내서.md)**의 "5. 추천 학습 방법" 섹션을 참조하세요.
