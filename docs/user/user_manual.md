...
*   **과목별 학습 상태**: 1과목부터 4과목까지 카드 수, 퀴즈 개수, 각 과목의 상세 진척도와 바로가기 버튼이 내장되어 있습니다. 과목별 **헷갈린 카드 수**와 **푼 퀴즈 개수**도 함께 표시되어 어느 과목에 집중해야 할지 한눈에 파악할 수 있습니다.
*   **모의고사 성적 분석 섹션**: 성적 추이 그래프, 과목별 역량 레이더 차트, 합격 가능성 진단 카드로 구성됩니다. 아직 모의고사를 풀지 않은 상태에서는 각 카드에 **"지금 모의고사 시작"** 버튼이 표시되어 바로 실전 모의고사로 진입할 수 있습니다.
*   **진도 초기화 (`top-right`)**: 클릭 시 확인 대화상자가 나타나며, 확인하면 로컬 스토리지에 보관된 학습 진도 데이터를 안전하게 포맷하고 대시보드를 `0%` 상태로 되돌립니다.
...
*   **11개 탭**: 테마 / 대시보드 / 카드 / 퀴즈 / 모의고사 / 훈련소 / 복습 / 교재 / 교재검색 / 성분검색 / 매뉴얼로 구성되어 있으며, 자주 사용하는 학습 메뉴가 앞쪽에 배치되어 있습니다. 가로 스크롤로 전체 메뉴에 접근할 수 있습니다.
...
## 16. 📖 교재 읽기 이어하기 (Reading Resume)

교재 리더에서 마지막으로 읽던 위치를 자동으로 저장하고, 앱을 다시 열었을 때 해당 위치부터 이어서 읽을 수 있는 기능입니다.
...
## 17. 🔄 간격 반복 학습 (Spaced Repetition — SM-2)

망각 곡선에 따라 카드를 잊기 전에 적시에 복습하도록 자동 스케줄링하는 기능입니다.

*   **SM-2 알고리즘**: 플래시카드 평가("외움"/"헷갈림") 시 SM-2 알고리즘이 다음 복습 시점을 자동 계산합니다.
*   **복습 간격 확장**: "외움"으로 평가한 카드는 1일 → 3일 → 7일 → 14일 → ... 순으로 복습 간격이 점진적으로 확장됩니다.
*   **간격 리셋**: "헷갈림"으로 평가하면 복습 간격이 1일로 리셋되어 다시 시작합니다.
*   **오늘 복습 카드 수**: 대시보드에 "오늘 복습" 카드 수가 표시되어, 오늘 복습해야 할 카드가 몇 장인지 한눈에 확인할 수 있습니다.
*   **진도 초기화 연동**: 대시보드의 "진도 초기화" 실행 시 간격 반복 스케줄도 함께 초기화됩니다.

> 💡 **참고**: 기존 "외움"/"헷갈림" 분류와 함께 동작합니다. 간격 반복은 복습 시점을 예측해 주는 보조 기능이며, 언제든 자유롭게 모든 카드를 학습할 수 있습니다.

---

## 18. 📊 학습 통계 및 약점 분석 (Learning Analytics)

대시보드에 과목별 정답률 시각화와 약점 과목 자동 추천 기능이 추가되었습니다.

### 과목별 정답률 히트맵
*   **색상 코딩**: 4개 과목의 퀴즈 정답률을 색상으로 한눈에 파악할 수 있습니다.
    *   🟢 80% 이상: 초록 (양호)
    *   🟠 60–79%: 주황 (보통)
    *   🔴 40–59%: 빨강 (주의)
    *   ⚫ 40% 미만: 진빨강 (집중 필요)
    *   ⚪ 미응시: 회색 (아직 퀴즈 미풀이)
*   **상세 툴팁**: 각 셀에 마우스를 올리면 과목명, 정답률, 푼 문제 수가 표시됩니다.

### 약점 과목 자동 추천
*   **정답률 최저 과목**: 3문 이상 푼 과목 중 정답률이 가장 낮은 과목을 자동 추천하며, "풀기" 버튼으로 바로 퀴즈를 시작할 수 있습니다.
*   **헷갈린 카드最多 과목**: 헷갈린 카드가 가장 많은 과목을 추천하며, "학습" 버튼으로 바로 플래시카드 학습을 시작할 수 있습니다.

---

## 19. ♿ 접근성 및 알림 개선 (Accessibility & Notifications)
...
## 20. 🔎 교재 검색 성능 개선 (Inverted Index Search)

교재 본문 검색이 역색인(inverted index) 방식으로 개선되어 더 빠르게 검색 결과를 확인할 수 있습니다.

*   **역색인 구축**: 교재의 모든 섹션 텍스트를 토큰화하여 단어→섹션 인덱스를 구축합니다.
*   **2-gram 보조 인덱스**: 한국어 특성을 고려하여 2글자 단위(bigram) 보조 인덱스를 함께 구성하여 부분 매칭도 빠르게 처리합니다.
*   **다중 키워드 교집합**: 여러 검색어를 입력하면 각 검색어의 후보 섹션 집합을 구하고 교집합을 계산하여 정확한 결과를 빠르게 반환합니다.
*   **자동 캐싱**: 최초 검색 시 역색인이 구축되며, 이후 같은 세션에서는 인덱스를 재사용하여 반복 검색이 더 빠릅니다.
