...
> **최종 업데이트**: 2026-09-25 · **버전**: v369 · [변경 이력](/docs/dev/CHANGES.md)
...
*   **⚙️ 설정 메뉴**: 화면 우측 상단의 톱니바퀴 버튼을 누르면 관리 기능이 한곳에 모여 있습니다 — 데이터 백업, 백업 가져오기, 시험 전환, 학습 매뉴얼·실무 매뉴얼, **학습/실무 모드 전환**, **앱 종료**, 진도 초기화. 메뉴는 항목을 선택하거나 바깥을 누르면 자동으로 닫힙니다.
...
*   **모바일**: 하단 탭 바로 모든 학습 메뉴에 접근하고, 백업·초기화·모드 전환 등 관리 기능은 상단의 ⚙️ 버튼에서 사용합니다.
...
*   **시험일 D-day**: 학습 캘린더의 "목표 설정"에서 시험일을 입력하면 대시보드 카드에 남은 일수가 표시되고, 현재 미학습 카드 수를 남은 일수로 나눈 **"역산 권장: 하루 N장"** 가이드가 함께 나타납니다. 시험이 7일 이내로 다가오면 강조 표시됩니다.
...
### 오늘의 합격 전략 (우선순위 추천)
학습 데이터를 종합해 **지금 가장 먼저 할 일**을 우선순위로 추천합니다. 각 추천에는 "왜 이걸 먼저 해야 하는지" 이유가 함께 표시되고, 버튼을 누르면 바로 해당 학습이 시작됩니다.

우선순위는 다음 순서로 결정됩니다:
1.  **오늘 복습할 카드** — SM-2 복습 시간이 도래한 카드가 있으면 최우선
2.  **과락 위기 과목** — 모의고사 과목별 점수가 60점 미만인 과목 집중 보강
3.  **정답률 최저 과목** — 3문 이상 푼 과목 중 정답률이 가장 낮은 과목
4.  **헷갈린 카드最多 과목** — 헷갈린 카드가 가장 많은 과목 복습
...
*   **CSV보내기**: 검색창 옆의 [CSV] 버튼으로 현재 검색·필터 결과를 CSV로 저장합니다 — 파일명에 DB 버전이 포함되며 UTF-8 BOM이라 Excel에서 바로 열립니다.
...
> 교재 본문 전체 텍스트 검색(장문 내용까지)은 §7의 "교재 본문 통합 검색" 메뉴를 이용하세요 — 팔레트는 빠른 이동·실행용입니다.
...
*   **시험일 등록 (D-day)**: 목표 설정 모달에서 시험일을 입력하면 대시보드에 D-day 카드와 "역산 권장 일일 카드 수"가 표시되고, 캘린더의 해당 날짜에는 **D-day 칩**이 붙습니다(7일 이내는 긴급 강조). 시험일이 지나면 대시보드 합격 진단 카드에 실제 결과 기록 폼이 열립니다.
...
### 실무 모드에서 달라지는 것
*   **학습 메뉴 숨김**: 대시보드·플래시카드·기출 퀴즈·훈련소·복습·모의고사·교재 읽기·교재 검색·학습 캘린더·학습 매뉴얼이 사이드바와 모바일 탭 바에서 사라집니다.
*   **학습 도구 펼침**: 데스크톱은 사이드바의 **[학습 도구 ▾]** 라벨, 모바일은 탭 바의 **[학습도구]** 탭을 누르면 숨겨진 학습 메뉴를 일시적으로 펼쳐 사용할 수 있습니다 (펼침 상태 기억·학습 모드 복귀 불필요).
*   **실무 중심 구성**: 실무 매뉴얼이 최상단으로 이동하고, 남는 메뉴는 실무 작업실·성분 검색 사전·테마·시험 전환뿐입니다. 모바일 탭 바는 14개 → **6개**(테마/실무/성분검색/실무매뉴얼/학습도구/시험전환)로 축소됩니다.
...
*   **14개 탭**: 테마 / 대시보드 / 카드 / 퀴즈 / 모의고사 / 훈련소 / 복습 / 교재읽기 / 교재검색 / 실무 / 성분검색 / 캘린더 / 매뉴얼 / 시험전환으로 구성되어 있으며, 자주 사용하는 학습 메뉴가 앞쪽에 배치되어 있습니다. 가로 스크롤로 전체 메뉴에 접근할 수 있고, 스크롤 가능한 방향 끝에는 그림자 힌트가 표시됩니다. 실무 모드에서는 학습 탭이 빠져 6개(테마/실무/성분검색/실무매뉴얼/학습도구/시험전환)로 축소되며, [학습도구] 탭으로 숨겨진 학습 메뉴를 펼칠 수 있습니다.
