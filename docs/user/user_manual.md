...
본 플랫폼은 **멀티시험 구조**로, 여러 자격시험 콘텐츠를 하나의 앱에서 병행 학습할 수 있습니다.

- **첫 실행 시**: 학습할 시험을 선택하는 화면이 표시됩니다. 시험 카드를 누르면 해당 시험의 대시보드로 진입합니다.
- **시험 전환**: 헤더 우측의 `⚙️ 설정 메뉴 → 시험 전환`(PC·모바일 공통) 또는 모바일 하단 탭 바 끝의 `시험전환` 탭으로 언제든 다른 시험으로 전환할 수 있습니다.
- **진도 격리**: 각 시험의 학습 진도·오답·통계는 완전히 분리되어 저장되므로, 시험을 오가며 학습해도 기록이 섞이지 않습니다. 테마 등 앱 공통 설정만 공유됩니다.
...
*   **⚙️ 설정 메뉴**: 화면 우측 상단의 톱니바퀴 버튼을 누르면 관리 기능이 한곳에 모여 있습니다 — 데이터 백업, 백업 가져오기, 시험 전환, 사용자 매뉴얼, 진도 초기화. 메뉴는 항목을 선택하거나 바깥을 누르면 자동으로 닫힙니다.
...
*   **실제 투입량 자동 계산**: 총 제조량(g/ml)과 원료별 배합률(%)을 입력하면 원료별 실제 투입량과 합계가 즉시 계산됩니다.
*   **합계 100% 판정**: 배합률 합계가 100%이면 🟢 '100% 정상', 부족하면 🟠 'N% 부족', 초과하면 🔴 'N% 초과' 배지가 합계 옆에 표시됩니다. 전성분 합계가 100%여야 완성 배합이 됩니다.
*   **제조 단계(Phase) 태그**: 원료 행마다 투입 단계를 지정할 수 있습니다 — 수상부 / 유상부 / 실리콘부 / 기능성 / 후첨가 / 기타. 합계 아래에 단계별 소계가 표시되고, '단계별 정렬' 버튼으로 행을 공정 순서로 자동 정렬합니다.
*   **목표·실측 pH**: 제조량 옆에 목표 pH와 실측 pH를 기록할 수 있습니다 (0~14).
...
*   계산기의 '제조 절차' 영역에 조제 순서를 단계별로 기록할 수 있습니다 (예: 수상부 80℃ 가열 → 유상부 용해 → 유화 → 후첨가). 최대 20단계, 포뮬러와 함께 저장되며 조제 기록지에 인쇄됩니다.
...
*   맞춤 조제 대상의 **고객명·나이·성별(남성/여성)·피부 유형·선택 제형**을 입력하고 **피부 고민·증상**을 칩으로 복수 선택할 수 있습니다.
...
*   고객 정보를 선택하면 그 아래에 **추천 패널**이 자동으로 나타납니다. 규칙 기반(고정 매핑) 엔진이며 AI 생성이 아닙니다.
...
*   **조제 기록지 인쇄**: 계산기 하단의 '조제 기록지 인쇄' 버튼으로 A4 출력용 기록지를 출력·PDF 저장할 수 있습니다. 포뮬러명·고객 정보·pH·원료 표(단계/배합률/투입량/검증 결과)·합계·제조 절차·메모가 포함되며, 하단에 안전 고지문이 인쇄됩니다.
*   **JSON 보내기**: 계산기의 'JSON 보내기'로 현재 작성 중인 포뮬러를, My 포뮬러 카드의 '보내기'로 저장된 포뮬러를 JSON 파일로보냅니다. 다른 기기·동료와 포뮬러를 공유할 수 있습니다.
*   **JSON 가져오기**: My 포뮬러 목록 상단의 'JSON 가져오기'로 공유받은 포뮬러 파일을 불러옵니다. Free 저장 한도(5개)가 그대로 적용됩니다.
*   맞춤 규칙은 이 기기의 로컬 저장소에 보관되며, **데이터 백업**에 포함됩니다.

### My 포뮬러 (저장)

*   **포뮬러로 저장**: 이름과 메모를 붙여 현재 배합 구성·고객 정보를 함께 저장합니다 (Free 최대 5개).
...
*   **13개 탭**: 테마 / 대시보드 / 카드 / 퀴즈 / 모의고사 / 훈련소 / 복습 / 교재 / 교재검색 / 성분검색 / 캘린더 / 매뉴얼 / 시험전환으로 구성되어 있으며, 자주 사용하는 학습 메뉴가 앞쪽에 배치되어 있습니다. 가로 스크롤로 전체 메뉴에 접근할 수 있습니다.
