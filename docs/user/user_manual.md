# 맞춤형화장품 조제관리사 스마트 통합 플랫폼 사용자 매뉴얼

> **최종 업데이트**: 2026-09-21 · **버전**: v369 · [변경 이력](/docs/dev/CHANGES.md)

본 스마트 통합 플랫폼은 맞춤형화장품 조제관리사 자격증 취득을 위한 법령 암기, 플래시카드, 계산식 연습, 실전 모의고사와 실무 배합 작업(Formula OS)을 한곳에서 수행할 수 있도록 설계된 프리미엄 SPA(Single Page Application) 통합 포털입니다.
...
*   **원료 데이터베이스 연동**: 워크스페이스 내의 원료 정보(`content/exams/cosmetic/참조자료/원료/` 폴더의 `approved_ingredients.md`, `restricted_ingredients.md`, `banned_ingredients.md`)를 자동으로 파싱하여 동적으로 문제를 생성합니다.
*   **4가지 핵심 학습 유형**:
...
*   **원료 DB 버전**: 화면 상단에 현재 원료 데이터베이스 버전(예: `원료 DB v2026.09.1`)이 표시됩니다. 법령·고시 개정으로 DB가 갱신되면 앱 접속 시 1회 알림이 표시되고 버전이 올라갑니다.
