...
- **시험 전환**: 데스크톱은 사이드바 하단의 `⇄ 시험 전환` 버튼 또는 헤더 우측의 `⚙️ 설정 메뉴 → 시험 전환`, 모바일은 하단 탭 바 끝의 `시험전환` 탭으로 언제든 다른 시험으로 전환할 수 있습니다.
...
*   **학습 진행 영구 소장**: 헤더 우측의 **⚙️ 설정 메뉴 → 데이터 백업**으로 지금까지 공부한 진척도 전체를 하나의 JSON 파일로 내보낼 수 있으며, 기기 변경이나 데이터 유실 시 **백업 가져오기**로 원클릭 복원이 가능합니다.
