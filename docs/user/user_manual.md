# Passmula — 맞춤형화장품 조제관리사 사용자 매뉴얼

> **최종 업데이트**: 2026-09-21 · **버전**: v369 · [변경 이력](/docs/dev/CHANGES.md)

Passmula는 맞춤형화장품 조제관리사 자격증 취득을 위한 법령 암기, 플래시카드, 계산식 연습, 실전 모의고사와 실무 배합 작업(Formula OS)을 한곳에서 수행할 수 있도록 설계된 프리미엄 SPA(Single Page Application) 통합 포털입니다.

### 🗂️ 메뉴 구성 (Navigation)

*   **사이드바 3그룹**: 데스크톱 좌측 메뉴는 용도별로 세 그룹에 정리되어 있습니다.
    *   **학습** — 대시보드 · 플래시카드 · 기출 퀴즈
    *   **훈련 · 평가** — 스마트 훈련소 · 오답/중요 복습 · 실전 모의고사
    *   **실무** — Formula OS (실전 배합 작업실)
    *   **자료 · 도구** — 교재 본문 읽기 · 교재 본문 검색 · 성분 검색 사전 · 학습 캘린더
*   **⚙️ 설정 메뉴**: 화면 우측 상단의 톱니바퀴 버튼을 누르면 관리 기능이 한곳에 모여 있습니다 — 데이터 백업, 백업 가져오기, 사용자 매뉴얼, 진도 초기화. 메뉴는 항목을 선택하거나 바깥을 누르면 자동으로 닫힙니다.
*   **모바일**: 하단 탭 바로 모든 학습 메뉴에 접근하고, 백업·초기화 등 관리 기능은 상단의 ⚙️ 버튼에서 사용합니다.
...
graph TD
    ...
    D -->|약점 집중 퀴즈| G
    H["7. 교재 리더 학습<br/>(학습보조·6대 학습요소·참조자료)"] --> G
    I["8. 오디오 학습<br/>(출퇴근길 청취)"] --> G
    J["9. 간격 반복 복습<br/>(SM-2 망각 곡선)"] --> G
```
...
*   **화면 구성**: 첫 화면에는 핵심 통계 카드, 일일 데일리 챌린지, 학습 안내서만 보입니다. 뽀모도로 타이머와 상세 분석(과목별 상태·정답률 히트맵·약점 추천·모의고사 예측)은 아래의 **"내 학습 분석 · 도구"** 바를 눌러 펼쳐서 확인합니다. 펼침/접힘 상태는 기기에 기억되어 다음 방문에도 유지됩니다.
...
*   **오늘 복습 카드 수**: SM-2 간격 반복 알고리즘이 계산한 오늘 복습 대기 카드 수입니다. 망각 곡선에 따라 잊기 전에 적시 복습하도록 자동 스케줄링됩니다.

### 내 학습 분석 · 도구 (접이식)

아래 항목들은 대시보드 하단의 **"내 학습 분석 · 도구"** 접이식 영역 안에 있습니다. 바를 눌러 펼치면 뽀모도로 타이머와 함께 전체 분석이 표시됩니다.
...
### 약점 과목 자동 추천
*   **정답률 최저 과목**: 3문 이상 푼 과목 중 정답률이 가장 낮은 과목을 자동 추천하며, **"퀴즈"** 버튼으로 바로 퀴즈를, **"교재"** 버튼으로 교재 읽기를 시작할 수 있습니다.
*   **헷갈린 카드最多 과목**: 헷갈린 카드가 가장 많은 과목을 추천하며, **"카드"** 버튼으로 바로 플래시카드 학습을, **"교재"** 버튼으로 교재 읽기를 시작할 수 있습니다.
*   📚 과목별 학습 우선순위와 시간 배분 추천은 **[학습 안내서](/content/exams/cosmetic/학습안내서.md)**의 "5. 추천 학습 방법" 섹션을 참조하세요.
...
*   **1~4과목 통합 실전 모의고사**: 실제 자격증 시험과 동일한 문항 출제 비율(1과목 10문항, 2과목 25문항, 3과목 25문항, 4과목 40문항, 총 100문항)에 맞춰 데이터베이스의 전체 1,000문항 중 무작위로 100문항을 조합해 매번 새로운 실전 시험지를 동적으로 출제합니다. 과목별 정답 및 오답 결과는 대시보드 역량 진단 레이더 차트 및 과락 진단 로직과 완벽히 호환됩니다. 출제 비율은 매니페스트(`content/exams/cosmetic/manifest.json`의 `integratedExam.questionsPerSubject`)에서 관리되어 과목별 문항 수 변경 시 자동 반영됩니다.
*   **과목별 집중 모의고사**: 각 과목별(1과목 100제, 2과목 250제, 3과목 250제, 4과목 400제, 총 1,000제) 완성도를 위해 분리 제공되는 모의고사 패키지입니다.
*   **ㄱㄴㄷㄹ 복수정답형 모의고사**: 각 과목 카드의 `복수정답형 모의고사` 버튼을 누르면 문항 수 선택 행(20/40/60/전체 N문)이 펼쳐집니다. 복수정답형 문항(과목별 102/250/250/408제)은 원본 문제은행 문항의 교재 근거를 ㄱ~ㅁ 진술 조합으로 자동 변환한 것으로, 실전 시험의 "옳은 것을 모두 고르시오" 패턴을 훈련합니다.
...
*   **원료 데이터베이스 연동**: 워크스페이스 내의 원료 정보(`content/exams/cosmetic/참조자료/원료/` 폴더의 `approved_ingredients.md`, `restricted_ingredients.md`, `banned_ingredients.md`)를 자동으로 파싱하여 동적으로 문제를 생성합니다. (같은 폴더의 `colorants_ingredients.md`는 색소 전용 참조 문서로, 사전·연습기 파싱 대상이 아닙니다.)
*   **4가지 핵심 학습 유형**:
...
*   **위치**: 대시보드의 **"내 학습 분석 · 도구"** 접이식 영역 안에 있습니다. 바를 눌러 펼치면 타이머가 나타납니다.
*   **25분 집중 세션**: 25분 집중 → 5분 휴식의 뽀모도로 기법을 지원합니다. 타이머가 백그라운드 스로틀링에 영향받지 않도록 `Date.now()` 기반으로 정밀하게 동작합니다.
*   **세션 카운트**: 오늘 완료한 집중 세션 횟수가 헤더에 표시되며, 일자별로 localStorage에 저장됩니다.
...
*   **Mermaid 다이어그램 렌더링**: 교재 본문에 포함된 Mermaid 마인드맵·플로우차트·시퀀스 다이어그램 등 12종이 자동으로 렌더링되어 시각적 학습을 지원합니다. 다이어그램이 있는 챕터를 열 때만 Mermaid.js(3.3MB)를 동적 로드합니다.
*   **이야기형 모드**: 단원 제목 우측의 토글 스위치로 표준형 ↔ 이야기형 교재를 전환할 수 있습니다. 이야기형은 주인공의 실수 경험을 통해 시험 함정을 회피하는 서사 구조로 학습 몰입도를 높입니다.
...
*   **자동 위치 저장**: 교재를 읽는 동안 스크롤 위치, 선택한 과목, 단원, 이야기형 모드 여부가 1초 간격으로 localStorage에 자동 저장됩니다.
...
*   **과목별 큐레이션 정의**: 과목별 JSON 파일에서 큐레이션된 전문 정의를 빌드 시 병합하여 표시합니다.
...
교재를 효율적으로 학습하기 위한 5단계 회독법 안내입니다. 각 과목의 `📋 목차`와 `🧠 30초 회상` 카드, `⚡ 시험 직전 체크리스트`와 함께 활용하세요.

> 📚 **학습 전략 상세 안내**: 출제비중 기반 우선순위 등급(S/A/B/C), 과목별 학습 순서, 시간 배분 추천 등은 **[학습 안내서](/content/exams/cosmetic/학습안내서.md)**의 "5. 추천 학습 방법" 섹션을 참조하세요.
...
*   **4개 단원 전체 오디오북 제공**: 1~4과목 전체 4개 단원의 교재 본문을 Google TTS 엔진으로 음성 합성한 오디오 MP3(총 약 11시간 분량)가 내장되어 있습니다. 출퇴근길이나 운전 중 등 눈으로 교재를 읽기 어려운 상황에서 귀로 학습할 수 있습니다.
...
*   **Media Session API 연동**: Android Chrome에서 오디오 재생 중 잠금화면/알림바에 미디어 컨트롤이 표시됩니다. 재생/일시정지/탐색/이전 단원/다음 단원 버튼으로 제어할 수 있습니다.
...
*   **원료 DB 버전**: 화면 상단에 현재 원료 데이터베이스 버전(예: `원료 DB v2026.09.1`)이 표시됩니다. 법령·고시 개정으로 DB가 갱신되면 앱 접속 시 1회 알림이 표시되고 버전이 올라갑니다. 배지를 탭하면 현재 버전과 과거 개정 이력을 모달로 확인할 수 있습니다.
...
## 9.5. 🧪 Formula OS — 실전 배합 작업실

시험 학습을 실무 배합으로 잇는 작업 공간입니다. 원료를 조회하고, 배합률을 계산하고, 고객 조건에 맞는 추천을 받고, 나만의 포뮬러를 저장·검증할 수 있습니다.

### 배합 계산기 (Blending Calculator)

계산기 화면은 **위·아래 고정 바 + 중앙 작업 영역** 구조로 되어 있습니다.

*   **상단 고정 요약바**: 총 제조량(g/ml) 입력과 배합률 합계·검증 요약이 스크롤과 무관하게 항상 보입니다.
*   **하단 고정 액션바**: 포뮬러 이름 입력과 저장·인쇄·JSON 버튼이 항상 표시되어, 목록이 길어져도 스크롤 없이 저장할 수 있습니다.
*   **실제 투입량 자동 계산**: 총 제조량과 원료별 배합률(%)을 입력하면 원료별 실제 투입량과 합계가 즉시 계산됩니다.
*   **합계 100% 판정**: 배합률 합계가 100%이면 🟢 '100% 정상', 부족하면 🟠 'N% 부족', 초과하면 🔴 'N% 초과' 배지가 상단 요약바에 표시됩니다. 전성분 합계가 100%여야 완성 배합이 됩니다.
*   **검증 요약**: 상단 요약바에 규정 검증 결과 건수(금지·초과·확인 필요 또는 정상)가 함께 표시됩니다.
*   **원료 행 카드**: 각 원료는 카드 형태입니다 — 위쪽 줄에 원료명·검증 배지·삭제 버튼, 아래쪽 줄에 배합률(%)·자동 계산된 투입량·제조 단계를 입력합니다.
*   **제조 단계(Phase) 태그**: 원료 행마다 투입 단계를 지정할 수 있습니다 — 수상부 / 유상부 / 실리콘부 / 기능성 / 후첨가 / 기타. 원료 목록 아래에 단계별 소계가 표시되고, '단계별 정렬' 버튼으로 행을 공정 순서로 자동 정렬합니다.
*   **원료 자동완성**: 원료명 입력 시 1,376종 데이터베이스에서 자동완성을 지원합니다.
*   **행 추가/삭제**: '원료 추가' 버튼으로 원료 행을 자유롭게 늘리고 개별 삭제할 수 있습니다.
*   **빈 상태 안내**: 입력된 원료가 없으면 안내 카드가 표시됩니다 — '추천 베이스 불러오기'(제형 미선택 시 세럼·에센스 기준)로 바로 시작할 수 있습니다.
*   **사전 연동**: 성분 사전 상세 카드의 '포뮬러에 추가' 버튼으로 원료를 계산기에 바로 투입할 수 있습니다.

### 제조 정보 (접이식 — pH·절차·메모)

*   계산기의 **'제조 정보' 섹션을 펼치면** 목표 pH·실측 pH(0~14), 제조 절차, 메모를 기록할 수 있습니다.
*   **제조 절차**: 조제 순서를 단계별로 기록합니다 (예: 수상부 80℃ 가열 → 유상부 용해 → 유화 → 후첨가). 최대 20단계, 포뮬러와 함께 저장되며 조제 기록지에 인쇄됩니다.
*   **메모**: 제조 조건, 보존 방법 등 자유 메모를 남길 수 있습니다.
*   접힌 상태에서도 제목 옆에 입력 내용 요약이 표시됩니다 (예: `pH 5.5/5.8 · 절차 4단계 · 메모`).

### 규정 검증 (Regulation Check)

*   원료 행마다 법정 배합 한도와 실시간 비교해 4가지 상태로 표시합니다.
    *   🟢 **한도 이내** — 입력한 농도가 고시 한도 이하
    *   🟠 **한도 초과** — 기준 한도 초과 (조건별 한도 확인 필요)
    *   🔴 **금지 원료** — 사용 금지 원료로 배합 불가
    *   ⚪ **확인 필요** — 한도 정보 해석 불가·농도 미입력·DB 미등록 (원문 확인 요망)
*   ⚠️ **중요**: '한도 이내'는 법정 기준 준수 확인일 뿐 제품의 안전성·안정성·품질을 보장하지 않습니다. 조건부 한도(씻어내는 제품·제품류별 한도 등)는 반드시 원문 고시를 확인하세요.

### 제형 안정성 체크 (Stability Check)

원료 행 아래에 **제형 안정성 패널**이 자동 표시됩니다. 규정 검증과 별개로, 배합비·배합방법이 제형 안정성에 미치는 영향을 규칙 기반으로 점검합니다.

*   **상(Phase) 비율**: 수상부+유상부인데 유화제가 없으면 분리 위험 경고, 유화제 비율이 유상부 대비 10% 미만이면 경고, 점증제 없는 에멀전은 크리밍·침강 참고 안내
*   **원료 상호작용**: 카보머×양이온성 원료, 음이온성×양이온성 계면활성제(침전), 비이온성 계면활성제 고농도(보존제 미셀 흡착)
*   **배합방법**: 열 민감 원료(비타민C·레티놀·히알루론산·향료 등)가 가열 단계에 있거나 절차에 가열이 기록되면 후첨가 권장, 카보머 중화 단계 누락, 에멀전인데 유화 단계 누락 안내
*   **pH 적정대**: 실측/목표 pH가 원료별 권장 범위(카보머 5.5~8·살리실산 3~4·비타민C 3.5 이하 등)를 벗어나면 경고
*   경고는 🟠 **주의**(분리·침전·변성 가능)와 ⚪ **참고**(개선 권고·기록 누락) 두 단계로 표시되며, My 포뮬러 카드에도 '안정성 n/참고 n' 배지로 요약됩니다. 조제 기록지 인쇄에도 포함됩니다.
*   ⚠️ 규칙에 없는 조합은 판정하지 않으며, 모든 안내는 참고용입니다 — 실제 안정성은 보존·가속 시험으로 최종 확인해야 합니다.

### 안정성 실험 확인

규칙 경고는 "가능성"일 뿐입니다 — 실제 안정성은 **실험으로만 확정**됩니다. '제조 정보' 섹션의 **안정성 실험 확인**에서 수행한 실험을 기록하세요.

*   **확인 방법**: 실온 경시 관찰 / 가속(고온) 시험 / 가혹 시험 / 동결-융해 시험 / 원심분리 시험 / 보존력 시험 / 기타
*   **결과**: 양호 / 이상 발견 + 메모 (예: "40℃ 4주 분리 없음") — **기록 일시는 저장 시 자동 부여**되며, 내용이 바뀔 때만 갱신됩니다.
*   기록하면 포뮬러 카드에 🟢 '안정성 확인' 또는 🔴 '안정성 이상' 배지가 표시되고, 안정성 패널 상단과 조제 기록지 인쇄에도 반영됩니다. JSON보내기·백업에도 포함됩니다.
*   **전성분 표시 자동 생성**: 결과가 **양호**로 확인된 배합은 저장 시 화장품법 전성분 표시 규칙(1% 초과 함량 내림차순 → 1% 이하 → 색소는 함량 무관 최하단)에 따라 **전성분 순서가 자동 생성·저장**됩니다. 포뮬러 카드와 조제 기록지 인쇄의 '전성분 표시' 행에서 확인할 수 있습니다.

### 고객 정보 (접이식 — 선택)

*   **'고객 정보' 섹션을 펼치면** 맞춤 조제 대상의 **고객명·나이·성별(남성/여성)·피부 유형·선택 제형**을 입력하고 **피부 고민·증상**을 칩으로 복수 선택할 수 있습니다. 접힌 상태에서도 제목 옆에 입력 요약이 표시됩니다 (예: `김OO · 34세 · 건성 · 알레르기 2종`).
    *   피부 유형: 건성 / 지성 / 복합성 / 중성 / 민감성
    *   피부 고민: 건조, 피지·모공, 여드름·트러블, 민감·홍조, 미백·잡티, 주름·탄력, 각질, 진정 (복수 선택)
    *   제형: 세럼·에센스, 토너·미스트, 로션·에멀전, 크림·밤, 젤, 오일, 클렌저, 선크림, 마스크·팩
*   **안전 관련 정보 (선택)** — 추천 엔진이 주의문·필터링에 활용합니다.
    *   **알레르기·부작용 이력 원료**: 원료명을 입력해 칩으로 추가하면 해당 원료가 모든 추천(기본·맞춤 후보 포함)에서 자동 제외되고 주의문에 표시됩니다.
    *   **임신·수유 여부**: '임신 중'/'수유 중' 선택 시 레티놀 등 비타민A 유도체·살리실산 계열 추천에 ⚠ 플래그와 전문의 상담 권장 주의문이 표시됩니다.
    *   **사용 중인 제품·약물**: 현재 사용 중인 제품·처방약(예: 레티놀 세럼, 스테로이드 연고)을 자유롭게 기록하면, 추천 원료와 이름이 겹칠 때 중복·병용 주의 주의문이 표시됩니다.
    *   이 정보들은 고객이 제공한 참고 정보이며 의학적 진단이나 안전 판정을 대신하지 않습니다.
*   고객 정보는 포뮬러와 함께 저장되며, My 포뮬러 목록 카드에 요약 표시됩니다. 모두 선택 항목이므로 비워 두어도 저장됩니다.

### 추천 베이스 · 원료 (Recommendation)

*   고객 정보를 선택하면 섹션 안에 **추천 패널**이 자동으로 나타납니다. 규칙 기반(고정 매핑) 엔진이며 AI 생성이 아닙니다.
*   **베이스 구성**: 선택한 제형에 필요한 역할(용제·보습제·오일·실리콘·유화제·점증제·계면활성제·보존제·pH 조절제·자외선차단제·산화방지제)을 순서대로 보여주고, 역할마다 5종 이상의 후보 칩을 제공합니다. `*` 표시는 수상 제형 필수 역할입니다.
*   **추천 원료**: 선택한 피부 고민·피부 유형에 맞는 기능성 원료를 이름과 이유 태그로 제안합니다. 사용 제한 원료는 `≤한도` 배지가 함께 표시되고, 20세 미만·65세 이상에는 자극 원료에 ⚠ 플래그가 붙습니다.
*   **칩 클릭 → 행 추가**: 후보 칩을 누르면 계산기에 원료명만 채워지고 제조 단계(Phase)도 자동 태깅됩니다 (예: 보존제→후첨가, 오일→유상부). 배합률(%)은 직접 입력하며 규정 검증이 그대로 적용됩니다.
*   **베이스 불러오기**: 필수 역할의 대표 후보를 한 번에 행으로 채웁니다.
*   ⚠️ 추천은 학습·작업 참고용 이름 제안이며, 농도는 제안하지 않습니다. 최종 조성은 고시 원문으로 확인하세요.

### 맞춤 추천 규칙 (Custom Rules)

*   계산기의 '맞춤 추천 규칙'을 펼치면 **내가 자주 쓰는 원료를 추천 후보에 직접 추가**할 수 있습니다.
*   **추가 방법**: 대상(베이스 역할 / 피부 고민 / 피부 유형)을 고르고 원료명을 입력해 '추가'를 누릅니다. 등록된 후보는 칩으로 표시되며 ✕로 개별 삭제할 수 있습니다.
*   **JSON 내보내기/가져오기**: 맞춤 규칙을 JSON 파일로 저장해 다른 기기·사용자와 공유하거나, 외부에서 작성한 규칙 파일을 불러올 수 있습니다.
    *   가져오기는 기본적으로 기존 맞춤 규칙에 **병합**되며, 중복·한도 초과분은 자동으로 건너뜁니다.
    *   파일 형식:
    ...
    *   `base`는 베이스 역할명, `concern`은 피부 고민 키, `skin`은 피부 유형 키를 사용합니다.
*   **안전 필터**: 사용 불가(banned) 원료나 DB에 없는 이름은 등록돼도 추천에 표시되지 않습니다.
*   **맞춤 규칙 초기화**: 등록한 맞춤 후보를 전부 지우고 기본 추천만 사용합니다 (기본 규칙은 삭제되지 않습니다).

### 조제 기록지 인쇄 · 포뮬러 공유

*   **조제 기록지 인쇄**: 하단 액션바의 🖨 인쇄 버튼으로 A4 출력용 기록지를 출력·PDF 저장할 수 있습니다. 포뮬러명·고객 정보·pH·원료 표(단계/배합률/투입량/검증 결과)·합계·제조 절차·메모가 포함되며, 하단에 안전 고지문이 인쇄됩니다.
*   **JSON 보내기**: 하단 액션바의 JSON 버튼으로 현재 작성 중인 포뮬러를, My 포뮬러 카드의 '보내기'로 저장된 포뮬러를 JSON 파일로보냅니다. 다른 기기·동료와 포뮬러를 공유할 수 있습니다.
*   **JSON 가져오기**: My 포뮬러 목록 상단의 'JSON 가져오기'로 공유받은 포뮬러 파일을 불러옵니다. Free 저장 한도(5개)가 그대로 적용됩니다.
*   맞춤 규칙은 이 기기의 로컬 저장소에 보관되며, **데이터 백업**에 포함됩니다.

### My 포뮬러 (저장)

*   **저장**: 하단 액션바에 이름을 입력하고 '저장'을 누르면 현재 배합 구성·고객 정보·제조 정보를 함께 저장합니다 (Free 최대 5개).
*   **목록 관리**: 저장된 포뮬러를 열어 수정하거나, 복제·삭제할 수 있습니다. 목록 카드에 규정 검증 요약과 고객 요약이 표시됩니다.
*   **규정 스냅샷**: 저장 시점의 한도 기준이 함께 보존되므로, 원료 DB가 갱신돼도 과거 포뮬러의 검증 근거가 유지됩니다.
*   저장된 포뮬러는 이 기기의 로컬 저장소에만 보관되며, **⚙️ 설정 메뉴 → 데이터 백업**에 자동 포함됩니다.
...
*   **월별 캘린더**: 학습한 날짜에 ✓ 스탬프가 표시됩니다. 오늘 날짜는 테두리로 강조됩니다.
...
*   **목표 설정**: "목표 설정" 버튼으로 일일 카드/퀴즈 목표와 주간 학습 일수를 조정할 수 있습니다.
*   **자동 기록**: 플래시카드 학습, 퀴즈 풀이 시 자동으로 날짜별 활동이 기록됩니다.
...
*   **13개 탭**: 테마 / 대시보드 / 카드 / 퀴즈 / 모의고사 / 훈련소 / 복습 / 교재 / 교재검색 / 포뮬러 / 성분검색 / 캘린더 / 매뉴얼로 구성되어 있으며, 자주 사용하는 학습 메뉴가 앞쪽에 배치되어 있습니다. 가로 스크롤로 전체 메뉴에 접근할 수 있습니다.
...
2. Chrome 앱을 직접 열어 주소창에 `personalized-skincare-study.vercel.app`을 입력한 후 설치합니다.
...
## 18. 🔧 localStorage 용량 초과 안내

*   브라우저 localStorage 용량(~5MB) 초과 시 화면 하단에 고정 경고 배너가 표시됩니다.
*   학습 진행 데이터 저장 실패 시 자동으로 배너가 활성화되어 데이터 유실 위험을 사전에 안내합니다.
