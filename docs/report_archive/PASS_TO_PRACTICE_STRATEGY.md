...
>
> **구현 상태 (2026-09-21)**: Phase 5-A는 구현·배포 완료. 이후 고객 안전 필드(알레르기·임신수유·사용 중 제품), 맞춤 추천 규칙, 제조 단계·pH·절차, 조제 기록지 인쇄, 포뮬러 JSON 공유, 원료 DB 버전 이력이 추가됨 — 상세는 `FORMULA_OS_DESIGN.md`와 `SPEC.md` §3.18 참조. 5-B 항목 중 "법령 업데이트 알림"의 일부(원료 DB 갱신 알림·이력 조회)는 5-A 범위 안에서 선구현됨.
