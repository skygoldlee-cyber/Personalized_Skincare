...
> **참고**: 향후 산업안전기사 등 다른 자격증으로 브랜드를 확장할 수 있습니다.
> 이는 부록(§A)을 참조하세요.
...
| **AI 플래시카드** | 교재에서 핵심 개념 자동 추출 (기존 `textbook-parser.js`) + 간격 반복 | 자동 추출은 이미 구현됨 + SM-2 망각 곡선 |
...
> **무료 사용자도 전 과목 학습 루프를 경험할 수 있어야 한다.**
> **잠금 축은 "과목 수"가 아닌 "깊이/분량"이다 — 4과목 모두 열되, AI·심층분석·무제한 분량은 Pro.**
> **Pro는 "개인화 + 장기 관리"로 전환 이유를 명확히 한다.**

> **리뷰 피드백 반영**: 4과목 중 1과목만 여는 것은 §3.1 정신에 위배됩니다.
> 이 도메인은 과목 간 교차참조가 강점이므로, "3과목이 안 보임"은 신뢰를 깎습니다.
> 전 과목을 열되 깊이로 잠급니다: 일일 문항 제한, AI 추천 없음, 심층 분석 없음.

### 3.2 Free / Pro 분담

| FREE — 전 과목 학습 루프 경험 (깊이 제한) | PRO — 개인화 + 장기 관리 (깊이 무제한) |
|---|---|
| 교재 전 과목 읽기 (분량 제한: 챕터당 1절) | 전 과목 교재 전체 분량 |
| 기본 플래시카드 (전 과목, 일일 30장) | 전 과목 플래시카드 무제한 |
| 기본 퀴즈 (일일 10문 제한) | 무제한 퀴즈 |
| 기본 오답 노트 (원인 분류만) | 심층 오답 분석 (원인→재학습 연결) |
...
> Free의 한도는 "전 과목 학습은 되지만 깊이가 제한된" 지점에 설정합니다.
...
1. "전 과목은 보이는데 분량이 제한되네, 전체 챕터가 보고 싶어"
...
> **Free/Pro**: 대부분 Free (AI 추천은 Pro 전용)
...
| **성공 지표** | 추천 카드 클릭률 ≥ 40% / 추천 따라간 세션의 정답률 개선 ≥ 10% |
...
| **Free** | 기본 오답 원인 분류 (3종) |
| **Pro** | 심층 분석 + 재학습 자동 연결 |
| **성공 지표** | 오답 후 재학습 세션 완료율 ≥ 50% / 재학습 후 동일 문제 재오답률 ≤ 30% |
...
| **성공 지표** | 멀티 디바이스 사용자 주 학습 시간 증가 ≥ 20% / 동기화 충돌률 ≤ 1% |
...
> **구현 상태 (2026-09-21)**: Phase 5-A 범위는 **Formula OS로 구현·배포 완료** — 원료 DB(성분 사전), 배합 계산기, My 포뮬러(저장 5개), 고시 한도 규정 검증. 추가 구현분: 고객 안전 필드·추천 규칙·제조 단계·인쇄·JSON 공유·원료 DB 갱신 알림. Pro/Supabase 항목(버전 관리·AI 배합·고객 관리·동기화)은 미구현 — 아래 목록의 "시점 C" 해당분이 이에 해당.
...
> **주의**: Phase 번호는 논리적 묶음이지, 구현 순서가 아닙니다.
> 실제 착수 순서는 **시점(A→C)**을 따릅니다.
> 시점 A(Zero-Backend 가능)는 Phase 1·2·4를 먼저 착수하고,
> 시점 C(Supabase 필요)는 Phase 3·5(일부)는 백엔드 준비 후 착수합니다.
>
> **⭐ 범례**: ⭐ = 합격률 임팩트 × 구현 우선순위 (⭐⭐⭐⭐⭐ = 최우선, ⭐⭐ = 부가)
...
3. **무료 사용자도 전 과목 학습 루프를 경험하게 한다 (잠금은 깊이로, 과목 수로 아님)**
...
> 본 문서의 대원칙은 "핵심 루프부터 완성"이므로, 아래는 참고용 방향성입니다.
...
- **COSMETIC PASS MASTER**: 맞춤형화장품조제관리사 (현재 집중)
- **SAFETY PASS MASTER**: 산업안전기사 (향후 확장 후보)
- 핵심 루프(Pass Core Loop) 엔진은 자격증 종류와 무관하게 재사용 가능
- 실무 확장(Practice Platform)은 도메인 특화이므로 각 자격증마다 별도 설계 필요
