...
export const ManualViewer = (() => {
    ...
    function _mdToHtml(mdText) {
        ...
        // 펜스 라인을 토큰으로 치환하되 언어 정보(```mermaid 등)를 보존
        ...
        const flushCode = () => {
            ...
            if (codeLang === 'mermaid') {
                // 머메이드 다이어그램: escapeHTML로 이스케이프된 원문을 그대로 넣어도
                // mermaid가 textContent를 읽으면 엔티티(--> 등)가 원래 문자(-->)로 복원됨
        ...
        lines.forEach(line => {
            ...
            if (trimmed.startsWith('```')) {
                if (inCodeBlock) { flushCode(); inCodeBlock = false; }
                    ...
                    inCodeBlock = true;
                    codeLang = trimmed.slice(3).trim().toLowerCase(); // ```mermaid → 'mermaid'
                }
                return;
            }
