...
function createIngredientCard(ing) {
    ...
    card.innerHTML = `
        ...
        <div class="dict-card-details is-hidden">
            ...
            <div class="dict-detail-item dict-detail-item--col"><span class="dict-detail-label">배합 한도</span><span class="dict-detail-value">${esc(ing.limit || '제한 없음')}</span></div>
            ${ing.tip ? `<div class="dict-card-tip">💡 <strong>TIP:</strong> ${esc(ing.tip)}</div>` : ''}
            ${hasFeature('formula') ? `<button class="btn btn-primary btn-sm dict-add-btn" data-click="formulaAddIngredient" data-arg="${esc(ing.name)}" title="배합 계산기 처방에 이 원료 추가"><i class="fa-solid fa-plus" aria-hidden="true"></i> 포뮬러에 추가</button>` : ''}
        </div>
    `;
