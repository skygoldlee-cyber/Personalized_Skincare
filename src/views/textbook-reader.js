...
// 참조자료 폴더의 파일 목록을 정적 목록으로 관리 (런타임 fetch 불가능하므로 하드코딩)
...
// 출처 텍스트 → PDF 매핑 (탐색 순서: 법령원문 → 공통 → 과목N)
// 모든 PDF 파일을 폴더별 레지스트리로 구성 (같은 파일명이면 법령원문 우선)
const _PDF_REGISTRY = {};
function _registerPdfDir(dir, files) {
    for (const f of files) {
        if (!_PDF_REGISTRY[f]) _PDF_REGISTRY[f] = dir; // 먼저 등록된 디렉토리가 우선
    }
}
// 우선순위 역순으로 등록 (나중에 등록한 것이 덮어쓰지 않음 → 먼저 등록한 법령원문이 우선)
_registerPdfDir('과목4', []);
_registerPdfDir('과목3', []);
_registerPdfDir('과목2', [
...
// 출처 키워드 → PDF 파일명 패턴 매핑
const _SOURCE_PDF_MAP = [
...
function _mapSourceToPdf(sourceText) {
    ...
    // 출처 텍스트에서 매칭되는 PDF 파일명 찾기
    let pdfFile = '';
    for (const entry of _SOURCE_PDF_MAP) {
        if (entry.exclude) {
    ...
    // 레지스트리에서 파일 위치 확인 (법령원문 → 공통 → 과목N 우선순위)
    const dir = _PDF_REGISTRY[pdfFile];
