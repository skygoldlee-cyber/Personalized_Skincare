...
// - statement_stats: 진술별 판정 누적 통계 { sid: { j, w, lw, t, truth, cid } } (약점 리스트용)
//   t=진술 텍스트, truth=정답 O/X, cid=conceptId(개념 그룹) — 취약 진술 리뷰 화면용 메타
//
...
export function recordStatementJudgments(perStatement) {
    ...
    for (const s of perStatement) {
        ...
        const cur = stats[s.sid] || { j: 0, w: 0, lw: null, t: '', truth: null, cid: null };
        cur.j += 1;
