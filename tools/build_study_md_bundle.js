...
const examsSrc = path.join(ROOT, 'content', 'exams.json');
const examsDoc = JSON.parse(fs.readFileSync(examsSrc, 'utf-8'));

for (const exam of examsDoc.exams) {
    const contentRoot = exam.contentRoot || 'content';
    const dataRoot = exam.dataRoot || 'data';
    const manifestPath = path.join(ROOT, contentRoot, 'manifest.json');
