...
const INGREDIENTS_DIR = path.join(WORKSPACE_DIR, 'ingredients');
const OUTPUT_FILE = path.join(WORKSPACE_DIR, 'data', 'ingredients_data.js');
...
function compileIngredients() {
    ...
    if (fs.existsSync(restrictedPath)) {
        ...
        parsed.forEach(item => {
            ...
            if (nameIdx !== -1) {
                const name = cleanText(item.cells[nameIdx]);
                if (name && name !== '원료명' && name !== '성분명') {
                    // Check if already in list as approved (avoid duplicates, but upgrade to restricted)
    ...
    // Save to file
    const jsContent = `// 자동 생성된 화장품 원료 데이터 파일입니다. 수정하지 마십시오.\nconst INGREDIENTS_DATA = ${JSON.stringify(list, null, 2)};\n`;
    fs.writeFileSync(OUTPUT_FILE, jsContent, 'utf-8');
    console.log(`파싱 완료! 원료 데이터가 ${OUTPUT_FILE} 에 저장되었습니다. 총 원료 수: ${list.length}`);
}
