#!/usr/bin/env node
/* ============================================================
 * tools/build/build_combo_drills.js
 * ------------------------------------------------------------
 * data/exams/subjectN.*.js (choice 문항)를 복수정답형(combo) 드릴 문항으로 변환한다.
 * docs/dev/reference/COMBO_STUDY_STRATEGY.md §2-①: single 문항의 각 보기를 진술 원자로 펼치고
...
async function buildForExam(target) {
  ...
  if (!DRY_RUN) {
    ...
    const idxBody = '// 자동 생성된 복수정답형 문항 수 인덱스입니다. 수정하지 마십시오. (tools/build/build_combo_drills.js)\n' +
