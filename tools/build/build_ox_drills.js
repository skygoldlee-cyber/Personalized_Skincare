#!/usr/bin/env node
/* ============================================================
 * tools/build/build_ox_drills.js
 * ------------------------------------------------------------
 * data/exams/subjectN.*.js (choice 문항)를 O/X 진위형 드릴 문항으로 펼친다.
 * docs/dev/reference/COMBO_STUDY_STRATEGY.md §4-①: combo의 진술·single의 보기를 참/거짓 원자로 변환해
...
const AUTOGEN_HEADER = '// 자동 생성된 O/X 드릴 데이터입니다. 수정하지 마십시오. (tools/build/build_ox_drills.js)';

/* ---------- 발문 분류 ---------- */

// 명제를 뒤집는 설명형 부정 (옳지 않은 설명을 고르는 발문)
const NEG_DESC_RE = /옳지 ?않은|맞지 않는|틀린|잘못된|올바르지 않은|적합하지 않은|바람직하지 않은|부적절한|맞는 것이 아닌|아닌|일치하지 않는|거리가 먼|다른 것은|다른 하나는/;
...
function topicContext(stem) {
  const t = String(stem || '')
    .replace(/\s+/g, ' ')
    .replace(/^다음( 중)?\s*/, '')
    // 판정 절 앞의 조사는 절 문법의 일부 — 함께 제거 ("특징이 아닌" → "특징")
    .replace(/(으로|로|중|이|가|은|는|에|에서)?\s*(가장\s*)?(옳지 ?않은|옳은|맞지 않는|틀린|적절하지 않은|바람직하지 않은|부적절한|올바르지 않은|잘못된|맞는|해당하는|해당하지 않는|아닌|다른|거리가 먼|일치하지 않는|맞도록 나열된|바르게 나열된)\s*것은\?.*$/u, '')
    .trim();
