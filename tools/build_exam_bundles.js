 ...
 * ------------------------------------------------------------
 * exams/*.md (실전 예상문제집 원본)을 클래식 <script>로 불러올 수 있는
 ...
 *          (window.__EXAM_MD__ = window.__EXAM_MD__ || {})["exams/<파일명>.md"] = "<마크다운>";
 *        키는 index.html의 onclick="ExamViewer.openExam('exams/...md')" 과 정확히 일치한다.
 *
...
const SRC_DIR = path.join(ROOT, 'exams');
...
function main() {
    ...
    console.log('  이후 exams/*.md 를 수정하면 이 스크립트를 다시 실행하세요.');
}
